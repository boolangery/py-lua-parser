...
cdef class Builder:
    ...
    cdef failure_save(self):
        self.failure()
        self.save()

    cdef bool next_is_rc(self, int type, bool hidden_right=True):
        cdef int tok_type
    ...
    cdef bool next_is_c(self, int type, bool hidden_right=True):
        cdef int tok_type
    ...
    cdef handle_hidden_left(self):
        ...
        if tokens:
            for t in tokens:
                ...
                elif token_type == Tokens.LINE_COMMENT:
                    self.comments.append(Comment(t.text))
                elif token_type == Tokens.COMMENT:
                    self.comments.append(Comment(t.text, True))


    cdef handle_hidden_right(self, is_newline=False):
        cdef int token_type
    ...
    cdef parse_var(self, is_stat=False):
        self.save()
        number_of_tail = 0
    ...
    cdef parse_atom(self):
        ...
        if self.next_is(Tokens.NUMBER) and self.next_is_rc(Tokens.NUMBER):
            ...
            try:
                number = literal_eval(str(self.text))
            except:
                # exception occurs with leading zero number: 002
                number = float(str(self.text))
    ...
    cdef parse_table_constructor(self, render_last_hidden=True):
        self.save()
